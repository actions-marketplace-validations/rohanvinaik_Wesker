...
def _match(name, base="query", base_stripped=None, generated="test_query.py",
           parent_dir="src", parent_qualified=None, partial_stems=frozenset()):
    return _name_matches_convention(
        base, base_stripped or base, generated, name,
        parent_dir, parent_qualified, set(partial_stems),
    )
...
def test_match_partial_stem():
    ...
    assert _match("test_navigate.py", base="query_navigate",
                  generated="test_x.py", partial_stems={"query", "navigate"}) is True


def test_match_parent_qualified():
    assert _match("test_wiki_config.py", base="config", generated="test_x.py",
                  parent_dir="wiki", parent_qualified="wiki_config") is True


def test_no_match():
    assert _match("test_unrelated.py", base="query", generated="test_query.py",
                  partial_stems={"query"}) is False
...
def test_discover_tests_layers(tmp_path):
    ...
    found = [p.rsplit("/", 1)[-1] for p in discover_tests(str(tmp_path), str(src), ["f"])]
    assert found[0] == "test_query.py"          # convention first
    assert set(found) == {"test_query.py", "test_misc.py"}  # impact/fallback add the rest
...
def test_load_cached_state_reads_report(tmp_path):
    ...
    wdir.mkdir()
    (wdir / "mutation_report.json").write_text('{"per_category": [{"category": "VALUE"}]}')
